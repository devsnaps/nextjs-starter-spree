...
function toTaxonPermalink(permalink: string): string {
  return permalink.startsWith(TAXONOMY_PREFIX)
    ? permalink
    : `${TAXONOMY_PREFIX}${permalink}`;
}
...
export default async function CategoryPage({ params }: CategoryPageProps) {
  ...
  const fullPermalink = toTaxonPermalink(permalink.join("/"));
          ...
          <div className="mb-8">
            ...
            <div className="flex flex-wrap gap-2">
              {taxon.children.map((child) => (
                <a
                  key={child.id}
                  href={`${basePath}/collections/${toCategorySlugPath(child.permalink)}`}
